...
if($PLAYER_INFO != false && $PLAYER_STATUS != false){
  if(isset($_POST['set_map_id']) && isset($_POST['check_story_id'])){ //選択されたマップ
    ...
    else if(isset($_POST['area_mode']) && $_POST['area_mode'] == 2){ //ギルドメンバー共有エリア
      $areaMode = $_POST['area_mode'];
      $playerAreaInstance = getGuildAreaInstance($pdo,$PLAYER_INFO['player_index_id'],$ENV);
    }
    else{
      //エラー
    ...
    { //現在居るエリアか
      ...
      $checkOpenMap = false;
      if($areaMode == 2 && $mapId == $define->GUILD_HOME_MAP_ID){ //ギルドホームの場合は強制許可
        $checkOpenMap = true;
      }else{
      ...
      if($checkOpenMap == true){
        ...
        $mapGuildBoardDatas = getMapGuildBoardDatas($pdo,$mapCharacterList,$mapEventDatas);
        $RESULT_JSON = $RESULT_JSON + array('result_map_guild_board_datas' => $mapGuildBoardDatas); //マップに配置されたギルドボードのマスターデータ
        ...
        $RESULT_JSON = $RESULT_JSON + array('player_guild_data' => $playerGuildData); //所属ギルドデータを取得
        $playerGuildMemberDatas = getGuildMemberDatas($pdo,$PLAYER_INFO['player_guild_index_id']);
        $RESULT_JSON = $RESULT_JSON + array('player_guild_member_datas' => $playerGuildMemberDatas); //ギルドメンバーデータを取得
      }
      ...
      case 9: //プレイヤールームイベント
      {
        ...
        if(isset($_POST['check_point']) && isset($_POST['map_event_master_id']) && isset($_POST['room_pos']) && isset($_POST['check_avatar_datas'])){
          ...
          if($checkEventTriggerData != false){ //正しいイベントデータが存在した場合
            ...
            //ルームに居るプレイヤーを取得
            $playerRoomDatas = array();
            if(isset($_POST['player_room_is_guild_room'])){ //ギルドルーム
              $playerRoomDatas = getGuildRoomDatas($pdo,$checkEventTriggerData['trigger_id'],$PLAYER_INFO['player_guild_index_id']);
            }
            else{ //通常
            ...
            $playerRoomChatLogs = array();
            if(isset($_POST['player_room_is_guild_room'])){ //ギルドルームの場合はギルドチャットログに変更
              $playerRoomChatLogs = getGuildChatLogs($pdo,$PLAYER_INFO['player_guild_index_id'],30);
            }
            else{ //プレイヤールームのチャットログを取得
            ...
            if($checkAvatarDatas != false){
              foreach ($playerRoomDatas as $plRoomData) {
                ...
                if($checkAvatar == false){ //一致するアバターが存在しなかった場合
                  ...
                  $resultAvatarData['player_avatar_data'] = array();
  ...
  if(isset($_POST['player_room_update'])){ //プレイヤールームの更新が行われた
    ...
    if(isset($_POST['player_room_update']['room_pos']) && isset($_POST['player_room_update']['map_event_trigger_id'])){
      ...
      $playerRoomDatas = array();
      if(isset($_POST['player_room_is_guild_room'])){ //ギルドルーム
        $playerRoomDatas = getGuildRoomDatas($pdo,$mapEventTriggerId,$PLAYER_INFO['player_guild_index_id']);
      }
      else{ //通常
  ...
  if(isset($_POST['set_quick_chat_text']) && isset($_POST['set_quick_chat_guild_room_guild_id'])){ //ギルドルームのクイックチャットの入力が行われた
    if($_POST['set_quick_chat_text'] != ""){ //チャットメッセージの送信が行われた
      $quickChatStampId = 0;
      if(isset($_POST['set_quick_chat_stamp_id'])) $quickChatStampId = $_POST['set_quick_chat_stamp_id'];
      insertGuildChat($pdo,$PLAYER_MASTER['player_name'],$PLAYER_INFO['player_index_id'],$PLAYER_INFO['player_guild_index_id'],$_POST['set_quick_chat_text'],$quickChatStampId);
      $resultSendChatMessage = array();
      $resultSendChatMessage['chat_text'] = $_POST['set_quick_chat_text'];
      $resultSendChatMessage['stamp_id'] = $quickChatStampId;
      $RESULT_JSON = $RESULT_JSON + array('result_send_quick_chat_guild_room' => $resultSendChatMessage);
    }
    $playerRoomChatLogs = getPlayerRoomChatLogs($pdo,$_POST['set_quick_chat_player_room_event_trigger_id'],30);
    $RESULT_JSON = $RESULT_JSON + array('player_room_chat_logs' => $playerRoomChatLogs);
  }
  ...
  if(isset($_POST['set_player_kill_execute']) && isset($_POST['set_player_kill_execute']['area_id']) && isset($_POST['set_player_kill_execute']['kill_player_index_id'])){ //プレイヤーの殺害を行った
    $errorId = 0;
    $resultInsertBattleInstance = -1;
    $playerAreaInstance = getPlayerAreaInstance($pdo,$PLAYER_INFO['player_index_id']);
    $areaMasterData = getAreaMaster($pdo,$_POST['set_player_kill_execute']['area_id']);
    $enemyPlayerInfo = getPlayerInfoForIndexId($pdo,$_POST['set_player_kill_execute']['kill_player_index_id'],true);
    $mapMasterData = false;
    if($areaMasterData != false) $mapMasterData = getMapMasterData($pdo,$areaMasterData['map_id']);
    if($playerAreaInstance != false
    && $areaMasterData != false
    && $mapMasterData != false
    && $enemyPlayerInfo != false){ //現在居るエリアだった エリアマスター、マップマスターが存在した
      if($enemyPlayerInfo['player_party_index_id'] != $PLAYER_INFO['player_party_index_id']){ //違うチーム
        $maxPartyMember = 5;
        $myKarmaRank = getKarmaRank($PLAYER_INFO['player_karma']);
        if($myKarmaRank < 0){ //自分のカルマ犯罪者以上
          $enemyKarma = getKarmaRank($enemyPlayerInfo['player_karma']);
          $playerPartyData = getPlayerParty($pdo,$PLAYER_INFO['player_party_index_id']);
          $checkPlayerPartyMember = getPartyMemberPlayerDatas($pdo,$PLAYER_INFO['player_party_index_id']);
          //犯罪者でなければ、パーティからメンバーを除外
          $playerPartyMember = array();
          foreach ($checkPlayerPartyMember as $plInfo) {
            $getKarmaRank = getKarmaRank($plInfo['player_karma']);
            if($getKarmaRank < 0) array_push($playerPartyMember,$plInfo);
          }
          $enemyPlayerPartyData = getPlayerParty($pdo,$enemyPlayerInfo['player_party_index_id']);
          $enemyPartyMember = getPartyMemberPlayerDatas($pdo,$enemyPlayerInfo['player_party_index_id']);
          $enemyMemberCount = count($enemyPartyMember); //現在のパーティメンバー数
          $addEnemySupportMemberNum = $maxPartyMember - $enemyMemberCount; //パーティに追加可能なメンバー数
          $matchingListSupportPlayer = array(); //敵チームの仲間(救援)プレイヤーのid配列
          $matchingListSupportNpc = array(); //仲間(救援)NPCのid配列
          $enemyPlayerIndexId = -1;
          $mapMasterId = -1;
          $areaMasterData = null;
          //敵パーティが4人以下で、殺害対象のプレイヤーカルマが一般以上だった場合、敵チームに救援プレイヤー、救援NPCを追加
          if(1 <= $addEnemySupportMemberNum && 0 <= $enemyKarma){
            $matchingList = searchPvpPlayer($pdo,$playerAreaInstance['area_id'],1); //救援仲間を検索
          ...
          $myTeamMemberCount = count($playerPartyMember);
          $enemyTeamMemberCount = count($enemyPartyMember) + count($matchingListSupportPlayer) + count($matchingListSupportNpc);
          if($maxPartyMember < $enemyTeamMemberCount) $enemyTeamMemberCount = $maxPartyMember;
          if(0 < $myTeamMemberCount && 0 < $enemyTeamMemberCount){ //両チーム最低1人以上、参加者が存在すれば戦闘可能
            //戦闘インスタンスの生成の準備
            $list = array();
            ...
            $ememyBattlePartyInstance = new BattlePartyInstance($pdo,1,$enemyPlayerPartyData);
            if(get_class($ememyBattlePartyInstance) == 'BattlePartyInstance' && $ememyBattlePartyInstance->init == true){
              $list['team_two_party_instance'] = $ememyBattlePartyInstance;
            }
            //チーム2(敵チーム)のエントリータイプデータ配列を生成
            $list['team_two_entry_type_datas'] = array();
            //敵プレイヤーメンバーを追加
            foreach ($enemyPartyMember as $ptPlData) {
              $addEntryType = new BattleEntryType(0,$ptPlData['player_index_id'],1);
            ...
            if(count($list['team_two_entry_type_datas']) < $maxPartyMember){
              for ($i=0; $i < count($matchingListSupportPlayer); $i++) {
                $addEntryType = new BattleEntryType(0,$matchingListSupportPlayer[$i],1);
                array_push($list['team_two_entry_type_datas'],$addEntryType);
                if($maxPartyMember <= count($list['team_two_entry_type_datas'])) break;
              }
            }
            if(count($list['team_two_entry_type_datas']) < $maxPartyMember){
              for ($i=0; $i < count($matchingListSupportNpc); $i++) {
                $addEntryType = new BattleEntryType(1,$matchingListSupportNpc[$i],1);
                array_push($list['team_two_entry_type_datas'],$addEntryType);
                if($maxPartyMember <= count($list['team_two_entry_type_datas'])) break;
              }
            }
            ...
            && count($list['team_two_entry_type_datas']) != 0){ //インスタンス生成可能かのチェック
              ...
              if($resultInsertBattleInstance['error'] == 0){ //エラーなし
                ...
                for ($i=0; $i < count($playerPartyMember); $i++) {
                  $msgTitle = "プレイヤーに攻撃を開始";
                  $msgText = $PLAYER_MASTER['player_name']."がプレイヤーに攻撃しました。";
                  insertPlayerMessage($pdo,3,$msgTitle,$msgText,$playerPartyMember[$i]['player_index_id'],$resultInsertBattleInstance['battle_instance_id'],"",""); //殺害結果のメッセージを自分に送信
                }
                $enemyPlayerName = getPlayerName($pdo,$enemyPlayerInfo['player_index_id']);
                for ($i=0; $i < count($enemyPartyMember); $i++) {
                  $msgTitle = "プレイヤーから攻撃を受けた";
                  $msgText = $enemyPlayerName."がプレイヤーから攻撃を受けました。";
                  if($enemyPartyMember[$i]['player_index_id'] == $enemyPlayerInfo['player_index_id']) $msgText = "プレイヤーから攻撃を受けました。";
                  insertPlayerMessage($pdo,3,$msgTitle,$msgText,$enemyPartyMember[$i]['player_index_id'],$resultInsertBattleInstance['battle_instance_id'],"",""); //殺害結果のメッセージを自分に送信
                }
                for ($i=0; $i < count($matchingListSupportPlayer); $i++) {
                  $msgTitle = "救援対象発見";
                  $msgText = "プレイヤーの救援を開始しました。";
                  insertPlayerMessage($pdo,3,$msgTitle,$msgText,$matchingListSupportPlayer[$i],$resultInsertBattleInstance['battle_instance_id'],"",""); //殺害結果のメッセージを自分に送信
                }
              } else $errorId = 6;
            } else $errorId = 5;
          } else $errorId = 4;
    ...
    $errorComment = "";
    switch ($errorId) {
      case 1:{$errorComment = "エリアデータ、マップデータの取得に失敗しました。";}
      break;
      case 2:{$errorComment = "同じパーティに所属しているため、戦えません。";}
      break;
      case 3:{$errorComment = "犯罪者ではないため、プレイヤーの殺害は行えません";}
      break;
      case 4:{$errorComment = "チームデータの処理に失敗しました。";}
      break;
      case 5:{$errorComment = "戦闘インスタンス生成の準備に失敗しました。";}
      break;
      case 6:{$errorComment = "戦闘インスタンスの生成に失敗しました。";}
      break;
      default:
      break;
    }
    $resultPlayerKillExecute = array();
    $resultPlayerKillExecute['error'] = $errorId;
    $resultPlayerKillExecute['error_comment'] = $errorComment;
    $resultPlayerKillExecute['battle_instance_id'] = $resultInsertBattleInstance['battle_instance_id'];
